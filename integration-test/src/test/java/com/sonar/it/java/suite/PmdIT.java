...
class PmdIT {
    ...
    void testP3cRules() {
        ...
        // given
        final String projectName = "pmd-p3c";
        final MavenBuild build = MavenBuild
                .create(TestUtils.projectPom(projectName))
                .setGoals("clean", "compile", "sonar:sonar");

        ORCHESTRATOR.associateProjectToQualityProfile("pmd-p3c", projectName);

        // when
        ORCHESTRATOR.executeBuild(build);

        // then
        final List<Issue> prodIssues = retrieveIssues(keyFor(projectName, "", "Bad_class_name"));
        assertThat(prodIssues).hasSize(2);
        assertThat(prodIssues.get(0).ruleKey()).isEqualTo("pmd:ClassNamingShouldBeCamelRule");
